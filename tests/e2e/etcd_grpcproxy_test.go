...
func TestGrpcProxyAutoSync(t *testing.T) {
	...
	// add a new member
	err = epc.MemberAdd(ctx, t)
	require.NoError(t, err)
	// start the new member
	err = epc.MemberStart(ctx, epc.Cfg.ClusterSize-1)
	require.NoError(t, err)
	...
	// stop the newly added member
	err = epc.MemberStop(epc.Cfg.ClusterSize - 1)
	require.NoError(t, err)
	// remove the newly added member
	epc.MemberRemove(epc.Cfg.ClusterSize - 1)
