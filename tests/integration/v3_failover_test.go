	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for retryCount > 0 {
				// put data test
				err = func() error {
					t.Logf("Sanity test, putting data [%s]", tc.name)
				...
				// get data  test
				err = func() error {
					t.Logf("Sanity test, getting data [%s]", tc.name)
					ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
					defer cancel()
					...
					if !bytes.Equal([]byte(val), resp.Kvs[0].Value) {
						t.Errorf("Unexpected value, expected: %s, got: %s", val, string(resp.Kvs[0].Value))
					}
					return nil
				}()
