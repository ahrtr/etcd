...
func (b *bootstrappedRaft) newRaftNode(ss *snap.Snapshotter, wal *wal.WAL, consistIndex cindex.ConsistentIndexer, cl *membership.RaftCluster) *raftNode {
	var n raft.Node
	if len(b.peers) == 0 {
	...
	return newRaftNode(
		raftNodeConfig{
			...
			storage:     serverstorage.NewStorage(b.lg, wal, ss, consistIndex),
		},
	)
}
