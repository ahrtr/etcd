...
func TestCheckCluster(t *testing.T) {
	...
	for i, tc := range cases {
		...
		if err == nil {
			...
			if clsInfo.Len() != len(expectedMemberInfo) {
				t.Errorf("case %d failed, unexpected member count, expected: %d, got: %d", i, len(expectedMemberInfo), clsInfo.Len())
			}
...
func TestRegisterSelf(t *testing.T) {
	...
	for i, tc := range cases {
		...
		if err := d.registerSelf(tc.expectedRegValue); err != nil {
			t.Errorf("case %d failed, error occuring on register member self: %v", i, err)
		}

		if fkv.retries != 0 {
			t.Errorf("case %d failed, discovery client did not retry registering itself on error, remaining retries: %d", i, fkv.retries)
		}
	}
}
	...
	for i, tc := range cases {
		...
		if err == nil {
			if retStr != tc.expectedResult {
				t.Errorf("case %d failed, expected result: %s, got: %s", i, tc.expectedResult, retStr)
			}
		}
	}
}
