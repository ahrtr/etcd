...
type Backend interface {
	...
	// CompactionRLock must be held by the KV store for the full duration of a compaction
	// pass. It prevents a compaction from running concurrently with a non-blocking
	// Defrag(): otherwise, keys removed by the compaction could be carried over into the
	// defragmented db, causing this etcd server's db hash to differ from other members'.
	CompactionRLock()
	CompactionRUnlock()
}
...
type backend struct {
	...
	defragMu sync.Mutex
	// compactionMu excludes Compact() from running concurrently with a non-blocking
	// Defrag() call; see CompactionRLock.
...
func (b *backend) defragNonBlocking() error {
	// Exclude Compact() for the whole call: its deletes from the "key" bucket (the only
	// safe-range bucket) would otherwise be invisible to catchUpDefrag's incremental,
	// append-only resync, which only detects keys added after the bulk-copy phase's
	// snapshot, never ones removed by a compaction that ran during it.
	b.compactionMu.Lock()
	defer b.compactionMu.Unlock()
	...
	if err := catchUpDefrag(b.db, tmpdb, lastKeys); err != nil {
		tmpdb.Close()
		if rmErr := os.RemoveAll(tmpdb.Path()); rmErr != nil {
			b.lg.Error("failed to remove db.tmp after non-blocking defragmentation completed", zap.Error(rmErr))
		}
...
func defragdbAndTrackLastKeys(odb, tmpdb *bolt.DB, limit int) (lastKeys map[string][]byte, err error) {
	...
	for next, _ := c.First(); next != nil; next, _ = c.Next() {
		...
		if err = b.ForEach(func(k, v []byte) error {
			...
			lastKeys[bucketName] = append([]byte(nil), k...)
			return tmpb.Put(k, v)
		}); err != nil {
			return nil, err
		}
...
func catchUpDefrag(odb, tmpdb *bolt.DB, lastKeys map[string][]byte) (err error) {
	tx, err := odb.Begin(false)
	if err != nil {
		return err
	}
...
func (b *backend) CompactionRLock() {
	b.compactionMu.RLock()
}

func (b *backend) CompactionRUnlock() {
	b.compactionMu.RUnlock()
}
