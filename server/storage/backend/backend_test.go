...
func TestBackendDefragBlockingAndNonBlockingProduceSameSize(t *testing.T) {
	...
	require.Equalf(t, bBlocking.Size(), bNonBlocking.Size(), "pre-defrag sizes should already match: both backends were populated identically")
	...
	require.NoError(t, err)
	assert.Equal(t, blockingHash, nonBlockingHash)
}
